splinePHGH.fit <- 
  function (x, y, id, initial.values, parameterization, derivForm, control, transform.value) # modified
  {
    ...
    if (control$typeGH != "simple") {
      lis.b <- vector("list", n)
      for (i in 1:n) {
        lis.b[[i]] <- t(control$inv.chol.VCs[[i]] %*% t(b)) + rep(control$ranef[i, ], each = k)
      ...
      for (i in seq_along(logT)) {
        ...
        if (parameterization %in% c("slope", "both") && (length(indRandom) > 1 || indRandom)) {
    ...
    for (it in 1:iter) {
      ...
      T.mat[it, ] <- switch(parameterization, 
                            value = c(gammas, alpha, gammas.bs),
      ...
      post.b <- if (control$typeGH == "simple") {
        ...
        sapply(seq_len(ncz), function(i) (p.byt * t(sapply(lis.b, "[", seq_len(k), i))) %*% wGH)
      }
      post.vb <- if (control$typeGH == "simple") {
        ...
        dd <- sapply(seq_len(ncz^2), function(i) (p.byt * t(sapply(lis.b2, "[", seq_len(k), i))) %*% wGH)
      ...
      list.thetas <- list(gammas = gammas, alpha = alpha, 
                          Dalpha = Dalpha, gammas.bs = gammas.bs)
      list.thetas <- list.thetas[!sapply(list.thetas, is.null)]
      thetas <- unlist(as.relistable(list.thetas))
      optz.surv <- optim(thetas, opt.survSplinePH, gr.survSplinePH, 
                         method = "BFGS", 
                         control = list(maxit = if (it < 5) 20 else 4,
                                        parscale = if (it < 5) rep(0.01, length(thetas))
                                        else rep(0.1, length(thetas))))
      betas <- betasn
      sigma <- sigman
      D <- Dn
    ...
    if (conv && !control$only.EM) {
      ...
      out <- if (control$optimizer == "optim") {
        ...
        nlminb(thetas, LogLik.splineGH, Score.splineGH, 
    ...
    list(coefficients = list(betas = betas, sigma = sigma, gammas = gammas, 
         ...
         logLik = lgLik,
         EB = list(post.b = post.b, post.vb = post.vb,
