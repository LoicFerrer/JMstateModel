  ...
  function (thetas) # modified
  {
    ...
    scalpha <- if (parameterization %in% c("value", "both")) {
      ...
      for (l in seq_along(rr)) {
        rrr <- {
          if (is.null(transform.value))
            exp.eta.tw.P * rowsum(Int * Ws.intF.vl[, l] * Ys, id.GK, reorder = FALSE)
          else exp.eta.tw.P * rowsum(Int * Ws.intF.vl[, l] * transform.value(Ys), id.GK, reorder = FALSE)
        }
        rrr <- rowsum(rrr, idT, reorder = FALSE)
        rr[l] <- {
          if (is.null(transform.value))
            -sum((p.byt * (rowsum(d * WintF.vl[, l] * Y, idT, reorder = FALSE) - rrr)) %*% wGH, na.rm = TRUE)
          else
            -sum((p.byt * (rowsum(d * WintF.vl[, l] * transform.value(Y), idT, reorder = FALSE) - rrr)) %*% wGH, na.rm = TRUE)
        }
      }
      rr
    }
    ...
    scalpha.D <- if (parameterization %in% c("slope", "both")) {
      ...
      for (l in seq_along(rr)) {
        rrr <- exp.eta.tw.P * rowsum(Int * Ws.intF.sl[, l] * Ys.deriv, id.GK, reorder = FALSE)
        rrr <- rowsum(rrr, idT, reorder = FALSE)
        rr[l] <- -sum((p.byt * (rowsum(d * WintF.sl[, l] * Y.deriv, idT, reorder = FALSE) - rrr)) %*% wGH, na.rm = TRUE)
      }
      rr
    }
    else NULL
