    ...
    if (method %in% c("Cox-PH-GH", "weibull-PH-GH", "piecewise-PH-GH", 
      ...
      if (!LongFormat) {
        ...
        if (!is.null(long)) {
          DD$long <- {
            if (is.null(transform.value))
              long * WintF.vl[id, , drop = FALSE]
            else transform.value(long) * WintF.vl[id, , drop = FALSE]
          }
          k <- ncol(DD$long)
        }
        if (!is.null(long.deriv)) {
        ...
        if (!is.null(long)) {
          DD$long <- {
            if (is.null(transform.value))
              as.vector(long.id[idT]) * WintF.vl
            else as.vector(transform.value(long.id[idT])) * WintF.vl
          }
          k <- ncol(DD$long)
        }
        if (!is.null(long.deriv)) {
