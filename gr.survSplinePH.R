  ...
  function (thetas) # modified
  {
    ...
    eta.t <- switch(parameterization,
                    ...
                    both = {
                      ...
                      else eta.tw2 + eta.tw1 + c(WintF.vl %*% alpha) * transform.value(Y) + c(WintF.sl %*% Dalpha) * Y.deriv
                    })
    eta.s <- switch(parameterization,
                    ...
                    both = {
                      ...
                      else c(Ws.intF.vl %*% alpha) * transform.value(Ys) + c(Ws.intF.sl %*% Dalpha) * Ys.deriv
                    })
    ...
    scalpha <- if (parameterization %in% c("value", "both")) {
      ...
      for (k in seq_along(rr)) {
        rrr <- {
          if (is.null(transform.value))
            exp.eta.tw.P * rowsum(Int * Ws.intF.vl[, k] * Ys, id.GK, reorder = FALSE)
          else exp.eta.tw.P * rowsum(Int * Ws.intF.vl[, k] * transform.value(Ys), id.GK, reorder = FALSE)
        }
        rrr <- rowsum(rrr, idT, reorder = FALSE)
        rr[k] <- {
          if (is.null(transform.value))
            -sum((p.byt * (rowsum(d * WintF.vl[, k] * Y, idT, reorder = FALSE) - rrr)) %*% wGH, na.rm = TRUE)
          else -sum((p.byt * (rowsum(d * WintF.vl[, k] * transform.value(Y), idT, reorder = FALSE) - rrr)) %*% wGH, na.rm = TRUE)
        }
      }
      rr
    }
    ...
    scalpha.D <- if (parameterization %in% c("slope", "both")) {
      ...
      for (k in seq_along(rr)) {
        rrr <- exp.eta.tw.P * rowsum(Int * Ws.intF.sl[, k] * Ys.deriv, id.GK, reorder = FALSE)
        rrr <- rowsum(rrr, idT, reorder = FALSE)
        rr[k] <- -sum((p.byt * (rowsum(d * WintF.sl[, k] * Y.deriv, idT, reorder = FALSE) - rrr)) %*% wGH, na.rm = TRUE)
      }
      rr
    }
    else NULL
